/-!
# Fully Verified Core Wisdom within Edge of Criticality
-/
...
-- Core Wisdom: persistently aligned subset of the reference liquid L
...
-- Key lemma: alignment window is independent of starting board iteration
...
-- Key lemma: adding subsystems does not decrease the alignment window
...
theorem core_wisdom_within_eoc
  ...
  -- Persistent alignment on singleton (window independent of iteration)
  ...
  -- L' = L_max ∪ {x} has strictly larger capacity
  let L' := L_max ∪ S
  have hx_not_L' : x ∉ L_max := hx_not_max
  ...
  -- L' is aligned (superset of aligned singleton)
  ...
  -- L' contradicts capacity-maximality of L_max among aligned sets
