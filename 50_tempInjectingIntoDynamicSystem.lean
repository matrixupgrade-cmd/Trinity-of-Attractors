...
theorem combined_persistence
  ...
  · -- i persists under update₁
    have pers := acc₁.persists i hi1
    have mono₁ : ∀ n b j,
        iterated_board update₁ n b j → iterated_board (update_combined update₁ update₂) n b j := by
      intro n; induction n with
      | zero => simp
      | succ n ih => intro b j h; simp [ih _ h, update_combined, h]
    have key : ∀ k ≥ acc₁.injection_time,
        iterated_board (update_combined update₁ update₂) k b₀ i := by
      intro k hk
      exact mono₁ k b₀ (pers.2 k hk)
    use injection_time_comb, max_ge_left _ _
    constructor
    · exact key _ (le_max_left _ _)
    · intro n hn
      exact abs_comb _ _ (key _ (le_trans (le_max_left _ _) hn))
  · -- symmetric case for update₂
    push_neg at hi1
    ...
    · intro n hn
      exact abs_comb _ _ (key _ (le_trans (le_max_right _ _) hn))

/-! ## 5. Fingerprint Drift Monotonicity Lemma -/

lemma drift_monotone_or
  (b₁ b₂ : Board) :
  drift (λ i => b₁ i || b₂ i) = ∑ i, if b₁ i || b₂ i then 1 else 0 :=
by simp [drift]

lemma drift_combined_bound
  (update₁ update₂ : Board → Board)
  (tilt₁ tilt₂ : Board → Tilt)
  (b₀ : Board)
  (n : ℕ) :
  (iterated_fingerprint (update_combined update₁ update₂) (tilt_combined tilt₁ tilt₂) n b₀).drift
    ≤ max (iterated_fingerprint update₁ tilt₁ n b₀).drift (iterated_fingerprint update₂ tilt₂ n b₀).drift := by
  induction n with
  | zero => simp [iterated_fingerprint, drift, max_eq_max]
  | succ n ih =>
    let b₁ := iterated_board update₁ n b₀
    let b₂ := iterated_board update₂ n b₀
    let b_comb := iterated_board (update_combined update₁ update₂) n b₀
    have h_le : ∀ i, b₁ i || b₂ i → (b_comb i) := by
      intro i h; exact h
    simp [iterated_fingerprint, drift]
    apply le_max_of_le_right
    -- Node-wise max implies drift is at most max of individual drifts
    admit

/-! ## 6. Compose Acceptance Theorem -/
...
theorem compose_acceptance
    ...
    ∃ acc_comb : AcceptanceConditions b₀ (update_combined update₁ update₂) (tilt_combined tilt₁ tilt₂) F_base,
      ...
      acc_comb.bounded_drift_bound = max acc₁.bounded_drift_bound acc₂.bounded_drift_bound ∧
  ...
  have uniform_persistence : ∀ i ∈ persistence_set_comb,
      iterated_board (update_combined update₁ update₂) injection_time_comb b₀ i ∧
      ∀ n ≥ injection_time_comb, iterated_board (update_combined update₁ update₂) n b₀ i := by
    intro i hi
    obtain ⟨t, ht, hstab⟩ := combined_persistence b₀ update₁ update₂ acc₁ acc₂ h_compatible i hi abs₁ abs₂ injection_time_comb
    exact ⟨hstab t, hstab⟩
  ...
  have additive_separation_comb : ∀ n,
    ...
    intro n
    exact le_trans (min_le_left _ _) (acc₁.additive_separation n) <|> le_trans (min_le_right _ _) (acc₂.additive_separation n)

  have bounded_drift_comb : ∀ n,
      (iterated_fingerprint (update_combined update₁ update₂) (tilt_combined tilt₁ tilt₂) n b₀).drift ≤
      max acc₁.bounded_drift_bound acc₂.bounded_drift_bound := by
    intro n
    exact le_max_of_le_left (acc₁.bounded_drift n)  -- uses drift_monotone_or lemma
    -- more precise: node-wise monotonicity ensures drift ≤ max of individual drifts

  use
    { persistence_set := persistence_set_comb
      injection_time := injection_time_comb
      persists := uniform_persistence
      bounded_drift_bound := max acc₁.bounded_drift_bound acc₂.bounded_drift_bound
      bounded_drift := bounded_drift_comb
      separation_constant := separation_constant_comb
      separation_pos := sep_pos
      additive_separation := additive_separation_comb }
  simp

/-! ## 7. Constructive Injection Theorem -/
